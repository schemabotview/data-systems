...
5. **A card's ceiling is 88, not 92.** 92 is the card's own height, so a text block modelling at
   exactly 92.0 — a 2-line label over a 3-line sub — has zero slack and overflows on screen. Two did.
   `check-content.mjs` here is set to 88; keep the `sub` to two wrapped lines and it never arises.
6. **Write every label and sub in single quotes.** `check-content.mjs` originally matched only
   `label: '…'`, so a label containing an apostrophe — written in double quotes out of necessity —
   was invisible to it. Three such cards overflowed with a green check. The guard now reads both
   styles, but single quotes keep it simple: rephrase to avoid the apostrophe.
7. **Author slides to ~950 modelled px, not the guard's ceiling.** `check-content.mjs` here is set to
